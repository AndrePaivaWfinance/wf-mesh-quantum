...
export function Duvidas() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-surface-900">Dúvidas de Classificação</h1>
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-surface-500">
            <strong className="text-surface-900 font-semibold">{pendentes.length}</strong> pendentes
          </span>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <div className="w-3 h-3 rounded-full bg-green-500" />
          <span className="text-surface-500">
            <strong className="text-surface-900 font-semibold">{items.length - pendentes.length}</strong> resolvidas
          </span>
        </div>
      </div>
              ...
              <div className="p-5">
                      ...
                      <button
                        ...
                        className={clsx(
                          ...
                          selectedCategoria[item.id] === s.categoria
                            ? 'border border-primary-500 bg-primary-50 ring-1 ring-primary-500/30 rounded-xl'
                            : 'border border-surface-200 hover:border-surface-300 rounded-xl'
                        )}
                      >
                        ...
                        <div className="mt-1.5 flex items-center gap-2">
                          <div className="flex-1 h-1.5 bg-surface-100 rounded-full overflow-hidden">
                            <div
                              className={clsx(
                                'h-full rounded-full',
                                s.confianca > 0.5 ? 'bg-emerald-500' : s.confianca > 0.3 ? 'bg-amber-500' : 'bg-red-400'
                              )}
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-surface-100">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl shadow-sm text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <Check className="w-4 h-4" /> Confirmar Categoria
                  </button>
                  <button
                    onClick={() => handlePular(item.id)}
                    className="flex items-center gap-2 px-4 py-2 border border-surface-200 text-surface-500 hover:bg-surface-50 rounded-xl text-sm font-medium transition-colors"
                  >
